...
input1 = pd.read_csv('/home/yohai/Downloads/intervals_DATA.csv', sep='\t')
data = input1.as_matrix()
p = ggplot(aes(x='lit_val', y='number_of_days'), data=input1) + geom_point(size=16, color='blue')
print(p)
...
input1['linspace'] = pd.Series(np.linspace(-0.2, 1.2, 36), index=input1.index)
input1['fit'] = pd.Series(func(np.linspace(-0.2, 1.2, 36), *popt1), index=input1.index)
p = ggplot(aes(x='lit_val', y='number_of_days'), data=input1) + geom_point(size=16, color='blue')
# t = ggplot(aes(x='lit_val', y = 'number_of_days'), data = input1)  + geom_point()  + stat_smooth(aes(x='lit_val', y = 'number_of_days'), data=input1) + geom_line(aes(x='linspace' ,y='fit'), size=1, color='blue')
# print(t)
...
# plt.savefig('/home/yohai/Documents/סמינריון/fig1.png')
